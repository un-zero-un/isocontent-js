...
describe('JsonParser', () => {
    ...
    it('should parse text node', () => {
        assert.deepStrictEqual(
            ...
            new JSONParser().parse('{"type":"text","value":"foobar"}')
        );
    });
    ...
    it('should parse block node with props', () => {
        assert.deepStrictEqual(
            ...
            new JSONParser().parse(
                '{"type":"block","block_type":"title","arguments":{"level":4}}'
            )
        );
    });
    ...
    it('should parse block node with children', () => {
        assert.deepStrictEqual(
            ...
            new JSONParser().parse(
                '{"type":"block","block_type":"title","arguments":{"level":4}, "children":[{"type":"text","value":"foobar"}]}'
            )
        );
    });
});
